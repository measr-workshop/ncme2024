...
* Simulated Taylor Swift (and others) album assessment data
...
usethis::use_course("measr-workshop/ncme2024-exercises")
```
...
4. Explore the MDM and Taylor data and Q-matrices
...
:::{.column width="65%"}
```{r taylor-data, echo = TRUE, eval = FALSE}
taylor_data
```
```{r print-taylor-data, echo = FALSE, eval = TRUE}
print(taylor_data, width = 50)
```
:::
...
```{r taylor-strc, echo = TRUE}
#| class-output: highlight
#| output-line-numbers: "|9,11"

measr_extract(taylor_lcdm,
              "strc_param")
```
...
```{r taylor-1-3-results, echo = TRUE}
measr_extract(taylor_lcdm,
              "strc_param") |> 
  slice(c(6, 8)) |> 
...
:::{.fragment}
```{r extract-taylor-new, echo = TRUE}
