...
taylor_data <- example_dat |> 
  ...
  write_rds(here("materials", "slides", "data", "taylor-data.rds"))

taylor_qmatrix <- qmatrix |> 
  rename(songwriting = att1, production = att2, vocals = att3) |> 
  write_rds(here("materials", "slides", "data", "taylor-qmatrix.rds"))
...
album_meta <- here("materials", "slides", "figure", "images", "taylor") |> 
  dir_ls() |> 
  as_tibble() |> 
  rename(img = value) |> 
...
predict(taylor_dcm, summary = FALSE) |> 
  ...
  write_rds(here("materials", "slides", "data", "taylor-results.rds"))
